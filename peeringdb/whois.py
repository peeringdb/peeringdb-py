...
class WhoisFormat(object):
    def __init__(self, fobj=sys.stdout):
        ...
        self.display_names = {"fac_set": "Facilities"}
    ...
    def display_set(self, typ, data, columns):
        ...
        for each in data:
            row = tuple(self._get_val(each, k) for k, v in list(each.items()))
            self._print(fmt % row)
    ...
    def print_net(self, data):
        ...
        self.display_field(
            fmt, data, "policy_locations", "Location Requirement"
        )
        self.display_field(fmt, data, "policy_ratio", "Ratio Requirement")
        self.display_field(
            fmt, data, "policy_contracts", "Contract Requirement"
        )
        self._print("\n")
